...
export function maskSettingsSecretsForDisplay(settings: Settings): Settings {
  ...
  const maskBlock = (block: Record<string, unknown> | undefined) => {
    ...
    for (const key of SECRET_FIELD_NAMES) {
      if (!isBlank(block[key])) block[key] = '';
    }
  };
...
export function getIntegrationSecretHints(settings: Settings): IntegrationSecretHints {
  ...
  for (const [integrationKey, block] of Object.entries(settings.integrations || {})) {
    ...
    for (const key of SECRET_FIELD_NAMES) {
      if (key in record) fieldHints[key] = !isBlank(record[key]);
    }
    if (Object.keys(fieldHints).length > 0) hints[integrationKey] = fieldHints;
  }
...
export function getIntegrationSecretPreviews(settings: Settings): IntegrationSecretPreviews {
  ...
  for (const [integrationKey, block] of Object.entries(settings.integrations || {})) {
    ...
    if (Object.keys(fieldPreviews).length > 0) previews[integrationKey] = fieldPreviews;
  }
...
export function integrationBlockConfiguredWithHints(
  ...
  if (secretHints) {
    for (const [key, stored] of Object.entries(secretHints)) {
      if (stored && isBlank(withHints[key])) withHints[key] = '__stored__';
    }
  }
...
export function fillBlankSettingsFromDefaults(settings: Settings, defaults: Settings): Settings {
  return {
    ...
    socialLinks: fillRecordFromDefaults(settings.socialLinks as Record<string, unknown> | undefined, defaults.socialLinks as Record<string, unknown> | undefined) as Settings['socialLinks'],
    branding: fillRecordFromDefaults(settings.branding as Record<string, unknown> | undefined, defaults.branding as Record<string, unknown> | undefined) as Settings['branding'],
    colors: fillRecordFromDefaults(settings.colors as Record<string, unknown> | undefined, defaults.colors as Record<string, unknown> | undefined) as Settings['colors'],
    ...
    membershipAccess: { paidPlansEnabled: settings.membershipAccess?.paidPlansEnabled ?? defaults.membershipAccess?.paidPlansEnabled ?? false },
...
export function mergeSettingsUpdates(existing: Settings, updates: Partial<Settings>): Settings {
  ...
  if (updates.integrations) result.integrations = mergeSettingsIntegrations(existing.integrations, updates.integrations);
  if (updates.socialLinks) result.socialLinks = mergeShallowPreservingBlank(existing.socialLinks as Record<string, unknown> | undefined, updates.socialLinks as Record<string, unknown>) as Settings['socialLinks'];
  if (updates.branding) result.branding = mergeShallowPreservingBlank(existing.branding as Record<string, unknown> | undefined, updates.branding as Record<string, unknown>) as Settings['branding'];
  if (updates.membershipAccess) result.membershipAccess = { paidPlansEnabled: updates.membershipAccess.paidPlansEnabled ?? existing.membershipAccess?.paidPlansEnabled ?? false };
  if (updates.siteHosting) result.siteHosting = { ...(existing.siteHosting || { status: 'inactive', domain: 'abundantglobalclub.com', updatedAt: Date.now() }), ...updates.siteHosting, updatedAt: Date.now() };
  if (updates.colors) result.colors = mergeShallowPreservingBlank(existing.colors as Record<string, unknown> | undefined, updates.colors as Record<string, unknown>) as Settings['colors'];

  if (updates.youtubeSection) {
    result.youtubeSection = {
      ...(mergeShallowPreservingBlank(existing.youtubeSection as Record<string, unknown> | undefined, updates.youtubeSection as unknown as Record<string, unknown>) ?? existing.youtubeSection),
  ...
  if (updates.heroSliderConfig) {
    ...
    // Undefined means no slide update was supplied. [] is an intentional update: delete all slides.
    const keepExistingSlides = incomingSlides === undefined;
    const base = resolveHeroSliderConfig(existing.heroSliderConfig, keepExistingSlides ? (existing.heroSliderConfig?.slides ?? existing.heroSlider ?? []) : incomingSlides);

    result.heroSliderConfig = resolveHeroSliderConfig(
      { ...base, ...updates.heroSliderConfig, slides: keepExistingSlides ? base.slides : incomingSlides },
  ...
  if (updates.homePage) {
    ...
    result.homePage = {
      eventsSection: mergeShallowPreservingBlank(baseHome.eventsSection as Record<string, unknown>, incomingEvents as Record<string, unknown>) as HomePageContent['eventsSection'],
      featuresSection: {
        ...(mergeShallowPreservingBlank(baseHome.featuresSection as Record<string, unknown>, incomingFeatures as Record<string, unknown>) as HomePageContent['featuresSection']),
        cards: incomingFeatures.cards !== undefined ? incomingFeatures.cards : baseHome.featuresSection?.cards ?? defaultHome.featuresSection.cards,
      },
      partnersSection: {
        enabled: incomingPartners.enabled ?? baseHome.partnersSection?.enabled ?? defaultHome.partnersSection.enabled,
        title: incomingPartners.title || baseHome.partnersSection?.title || defaultHome.partnersSection.title,
        speed: incomingPartners.speed ?? baseHome.partnersSection?.speed ?? defaultHome.partnersSection.speed,
        direction: incomingPartners.direction ?? baseHome.partnersSection?.direction ?? defaultHome.partnersSection.direction,
        easing: incomingPartners.easing ?? baseHome.partnersSection?.easing ?? defaultHome.partnersSection.easing,
        pauseOnHover: incomingPartners.pauseOnHover ?? baseHome.partnersSection?.pauseOnHover ?? defaultHome.partnersSection.pauseOnHover,
        grayscale: incomingPartners.grayscale ?? baseHome.partnersSection?.grayscale ?? defaultHome.partnersSection.grayscale,
        showEdgeFade: incomingPartners.showEdgeFade ?? baseHome.partnersSection?.showEdgeFade ?? defaultHome.partnersSection.showEdgeFade,
        gap: incomingPartners.gap ?? baseHome.partnersSection?.gap ?? defaultHome.partnersSection.gap,
        logoHeight: incomingPartners.logoHeight ?? baseHome.partnersSection?.logoHeight ?? defaultHome.partnersSection.logoHeight,
        partners: incomingPartners.partners !== undefined ? incomingPartners.partners : baseHome.partnersSection?.partners ?? defaultHome.partnersSection.partners,
      },
      ctaSection: mergeShallowPreservingBlank(baseHome.ctaSection as Record<string, unknown>, incomingCta as Record<string, unknown>) as HomePageContent['ctaSection'],
    };
  }

  if (updates.legalPages) {
    const defaults = getDefaultLegalPages();
    result.legalPages = { ...defaults, ...(existing.legalPages || {}), ...updates.legalPages };
  }
